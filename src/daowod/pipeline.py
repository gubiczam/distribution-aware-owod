...
class PipelineConfig:
    ...
    def bridge_settings(self) -> export_cache.BridgeSettings:
        import sys

        return export_cache.BridgeSettings(
...
class PipelineResult:
    ...
    def headline(self) -> list[dict[str, object]]:
        return reporting.summarise_strategies(self.outputs.auc_rows)
...
def stage_preflight(config: PipelineConfig, mode: ExecutionMode) -> list[preflight.Check]:
    ...
    checks: list[preflight.Check] = [preflight.check_python()]
    checks += preflight.check_packages()
    needs_gpu = config.require_gpu and not config.existing_export
    checks += preflight.check_gpu(required=needs_gpu)
    if not config.existing_export:
        checks += preflight.check_prob_checkout(config.prob_repository)
        checks.append(
            preflight.check_bridge_cli(
                config.prob_repository, python_executable=config.python_executable or None
            )
        )
        checks += preflight.check_checkpoint(
            config.checkpoint,
        ...
        checks.append(
            preflight.check_disk(
                ...
                required_gb=max(
                    2.0,
                    preflight.estimate_export_gigabytes(
    ...
    checks += preflight.check_dataset(
        config.data_root,
        dataset=config.dataset,
        split_file=config.split_file or None,
    )
    preflight.require_all_pass(checks)
    return checks
...
def stage_splits(
    ...
    return export_cache.split_disjoint(available_ids, counts=counts, seed=config.seed)


def _available_ids(config: PipelineConfig) -> list[str]:
    if config.split_file:
        return preflight.read_image_ids(config.split_file)
    export = export_cache.load_export_file(config.existing_export)
...
def stage_export(
    ...
    *,
    ...
    result = export_cache.export_proposals(
    ...
    export = export_cache.load_chunks(result.paths())
...
def run_pipeline(
    ...
    *,
    ...
    result.checks = preflight.rows(checks)
    reporting.write_csv(output_dir / "preflight.csv", result.checks)
    ...
    reporting.write_json(output_dir / "export_report.json", result.export)
    ...
    reporting.write_json(output_dir / "leakage_report.json", result.leakage)
    ...
    if pilot is not None:
        ...
        reporting.write_csv(output_dir / "pilot_ablation.csv", list(payload.get("pilot_rows", [])))
        base_study = replace(
            base_study,
    ...
    stage_started = time.time()
    result.contrasts = reporting.headline_contrasts(result.outputs.auc_rows)
    arm_rows = reporting.arm_comparison(result.outputs.auc_rows)
    arm_rows_tail = reporting.arm_comparison(result.outputs.auc_rows, metric="tail_discovery_auc")
    counts = reporting.discovery_counts(result.outputs.strategy_rows)
    reporting.write_csv(output_dir / "arm_comparison_unknown.csv", arm_rows)
    reporting.write_csv(output_dir / "arm_comparison_tail.csv", arm_rows_tail)
    reporting.write_csv(output_dir / "discovery_counts.csv", counts)
    result.cost_rows = reporting.budget_to_reach(
        result.outputs.strategy_rows, target_recall=config.target_tail_recall
    )
    written = reporting.write_study_outputs(result.outputs, output_dir)
    reporting.write_csv(output_dir / "headline_contrasts.csv", result.contrasts)
    reporting.write_csv(output_dir / "cost_to_target.csv", result.cost_rows)
    reporting.write_csv(output_dir / "strategy_summary.csv", result.headline())
    reporting.write_csv(output_dir / "ablations.csv", result.ablation_rows)
    reporting.write_csv(output_dir / "severity_report.csv", result.severity_rows)
    result.figures = plots.render_all(
    ...
    summary = reporting.research_summary(
        mode=mode.as_dict(),
    ...
    reporting.write_json(
        output_dir / "run_manifest.json",
        {
            ...
            "environment": preflight.environment_report(),
    ...
    result.artifacts = reporting.verify_expected_files(output_dir, REQUIRED_ARTIFACTS)
    ...
    result.archive = reporting.bundle(
